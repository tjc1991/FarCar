package com.cldxk.app.breakrules;
...
public class RulesActivity extends EBaseActivity implements OnClickListener{

	
	private String defaultChepai = "吉"; // 粤=广东
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		weizhangIntent.putExtra("appId",636);// 您的appId
		weizhangIntent.putExtra("appKey", "0154fc3f8a5b58b16e203ba0472a3832");// 您的appKey
		...
		btn_cpsz.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				intent.setClass(RulesActivity.this, ShortNameList.class);
		...
		query_city.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				Intent intent = new Intent();
				intent.setClass(RulesActivity.this, ProvinceList.class);
		...
		btn_query.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (result) {
					intent.setClass(RulesActivity.this, WeizhangResult.class);
	...
	private boolean checkQueryItem(CarInfo car) {
		if (car.getCity_id() == 0) {
			Toast.makeText(RulesActivity.this, "请选择查询地", 0).show();
			return false;
		}

		if (car.getChepai_no().length() != 7) {
			Toast.makeText(RulesActivity.this, "您输入的车牌号有误", 0).show();
			return false;
		}
		...
		if (car.getCity_id() > 0) {
			...
			if (classno > 0) {
				if (car.getChejia_no().equals("")) {
					Toast.makeText(RulesActivity.this, "输入车架号不为空", 0).show();
					return false;
				}

				if (car.getChejia_no().length() != classno) {
					Toast.makeText(RulesActivity.this, "输入车架号后" + classno + "位",
							0).show();
					return false;
				}
			} else if (classno < 0) {
				if (car.getChejia_no().length() == 0) {
					Toast.makeText(RulesActivity.this, "输入全部车架号", 0).show();
					return false;
				}
			}
			...
			if (engineno > 0) {
				if (car.getEngine_no().equals("")) {
					Toast.makeText(RulesActivity.this, "输入发动机号不为空", 0).show();
					return false;
				}
				...
				if (car.getEngine_no().length() == 0) {
					Toast.makeText(RulesActivity.this, "输入全部发动机号", 0).show();
					return false;
				}
			}
			...
			if (registno > 0) {
				if (car.getRegister_no().equals("")) {
					Toast.makeText(RulesActivity.this, "输入证书编号不为空", 0).show();
					return false;
				}
				...
				if (car.getRegister_no().length() == 0) {
					Toast.makeText(RulesActivity.this, "输入全部证书编号", 0).show();
					return false;
				}
			}
			return true;
